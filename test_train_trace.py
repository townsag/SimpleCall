...
checkpoint_dir = "/Users/andrewtownsend/Desktop/SJSU/CS271/final_project/SimpleCall/dummy_checkpoints"
batch_size = 64
s2s = False
...
def main():
    ...
    with jax.profiler.trace("/Users/andrewtownsend/Desktop/SJSU/CS271/final_project/SimpleCall/tmp/jax-trace"):
        ...
        train_state, train_metrics = train_one_epoch(state=train_state, train_dataloader=dataloader_train, 
                alphabet=BASES_CTC, epoch=1, batches_per_epoch=cfg.num_batches_per_epoch, checkpoint_dir=checkpoint_dir)
        end = time.time()
        print(f"========== Total Epoch Train Time: {end - start} ==========")
        print(f"Train epoch: {1}, loss: {train_metrics['mean_epoch_loss']}, accuracy: {train_metrics['mean_epoch_accuracy'] * 100}")


        start = time.time()
        print(f"Starting evaluation for epoch: {1}")
        test_metrics = evaluate_model(state=train_state, test_dataloader=dataloader_validation, alphabet=BASES_CTC, epoch=1)
        print(f"\Validation epoch: {1}, loss: {test_metrics['loss']}, accuracy: {test_metrics['accuracy'] * 100}")
        end = time.time()
